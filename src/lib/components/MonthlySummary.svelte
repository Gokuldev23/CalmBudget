<script lang="ts">
	...
	import type { CategoryItem, Expense, Budget } from '$lib/types';
	...
	// undefined = not overridden yet → fall back to prop; null/Budget = local override
	let _savedBudget = $state<Budget | null | undefined>(undefined);
	let currentBudget = $derived(_savedBudget !== undefined ? _savedBudget : initialBudget);
		...
		<div class="mb-5">
			...
			<div class="bg-gray-50 rounded-lg p-3 space-y-2">
				...
				<div class="flex justify-between text-sm">
					...
					<span
						class="font-semibold {(remaining ?? 0) >= 0 ? 'text-emerald-600' : 'text-red-500'}"
					>
						{#if (remaining ?? 0) < 0}
